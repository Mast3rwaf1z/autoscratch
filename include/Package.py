...
class Package:
    ...
    def configure(self):
        ...
        if self.src == None: check_call(["rm", "-f", f'build/{self.src.split("/")[-1]}*']) if not isinstance(self.src, list) else [check_call(["rm", "-f", f'build/{src.split("/")[-1]}*']) for src in self.src]
