...
class Package:
    ...
    def configure(self):
        ...
        if not self.src == None: check_call(["rm", "-vf", f'build/{self.src.split("/")[-1]}']) if not isinstance(self.src, list) else [check_call(["rm", "-vf", f'build/{src.split("/")[-1]}']) for src in self.src]
